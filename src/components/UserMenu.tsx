  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (user) {
        ...
        supabase.from('perfiles').select('rol').eq('id', user.id).single().then(({ data }) => {
          ...
          <div className="absolute right-0 top-full mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-elevated z-50 py-2">
            ...
            {esAdmin && (
              <Link
                href="/coordinacion"
                onClick={() => setOpen(false)}
                className="w-full text-left px-3 py-2 text-sm text-[#8B2252] hover:bg-[#8B2252]/5 transition-colors flex items-center gap-2 font-medium"
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
